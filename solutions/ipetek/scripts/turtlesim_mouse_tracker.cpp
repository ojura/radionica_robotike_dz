...
class MouseTurtleController {
   ...
   public:
    MouseTurtleController() {
        ...
        mouse_pos_ = node_.subscribe("/mouse_position", 1, &MouseTurtleController::mouse_pos_callback, this);
        turt_pos_ = node_.subscribe("/turtle1/pose", 1, &MouseTurtleController::turtle_pos_callback, this);
        turt_vel_ = node_.advertise<geometry_msgs::Twist>("/turtle1/cmd_vel", 1);
        turt_pub_timer_ = node_.createTimer(ros::Duration(0.01), &MouseTurtleController::turt_pub_callback, this);
    }

   private:
    // CALLBACK FUNCTIONS
    void mouse_pos_callback(const geometry_msgs::Point::ConstPtr &msg) { last_mouse_pos_ = *msg; }
    void turtle_pos_callback(const turtlesim::Pose::ConstPtr &msg) { last_turtle_pos_ = *msg; }
    ...
    double get_distance(const geometry_msgs::Point &goal) {
        return sqrt(pow((goal.x - last_turtle_pos_.x), 2) + pow((goal.y - last_turtle_pos_.y), 2));
    }

    double get_ang_distance(const geometry_msgs::Point &goal) {
        double ang = atan2(goal.y - last_turtle_pos_.y, goal.x - last_turtle_pos_.x) - last_turtle_pos_.theta;
        if (ang < -M_PI)
            ang += 2 * M_PI;
